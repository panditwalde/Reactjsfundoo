    ...
    DisplayAllNotes(token)
      .then(Response => {
        ...
        Response.data.data.forEach(element => {
          if (element.pin === false && element.reminder !== null) {
            console.log(element);
            array.push(element);
          }
          console.log(array);
          if (element.pin === true && element.reminder !== null) {
            array1.push(element);
          }
        });
        this.setState({
    ...
    return (
      ...
      >
        ...
        this.state.pinUnpinArray.length !== 0 ? null : (
          ...
          >
            <AddAlertOutlinedIcon style={{ fontSize: "7.5rem" }} />{" "}
            <b>Your Reminder notes appear here</b>{" "}
          </div>
        )}
